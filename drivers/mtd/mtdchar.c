...
static int mtd_do_write_page_with_oob(struct file *file, struct mtd_info *mtd,
    uint64_t start, uint32_t data_len, uint32_t oob_len,
    void __user *ptr, uint32_t __user *retp)
{
    struct mtd_oob_ops ops;
    uint32_t retlen;
    uint32_t length = data_len + oob_len;
    int ret = 0;
    ...
    ops.oobbuf = ops.datbuf + data_len;
...
static int mtd_ioctl(struct file *file, u_int cmd, u_long arg)
{
	...
	case BLKRRPART:
	{
        ...
        case MEMWRITEPAGEWITHOOB:
            {
                struct mtd_page_oob_buf buf;
                struct mtd_page_oob_buf __user *buf_user = argp;

                if (copy_from_user(&buf, argp, sizeof(buf)))
                    ret = -EFAULT;
                else
                    ret = mtd_do_write_page_with_oob(file, mtd, buf.address, buf.data_len,
                            buf.oob_len, buf.data_ptr, &buf_user->retlen);
                break;
            }
