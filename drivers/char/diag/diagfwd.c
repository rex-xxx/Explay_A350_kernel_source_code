...
//lct.panguangyi add for NV access
struct write_buf_mark
{
    void * buf;
    int len;
    int flag;
};
static struct write_buf_mark wbmark = {0};

int smd_device_write(const void * buf,int len)
{
    if(buf==NULL)
        return -1;
    ...
    return	smd_write(driver->ch, buf, len);
}

int smd_device_read(void * buf,int len)
{
    if((wbmark.flag&0x1)==0x1)
    {
        int avail_len =smd_read_avail(driver->ch);
        if(avail_len>255)
            avail_len = 255;
        if(len<0)
            len = avail_len;
        else
            len=min(len,avail_len);

        if(len>0)
        {
            smd_read(driver->ch, buf,avail_len);
            goto __exit_normal;
        }
        goto __exit_err;
    }else
