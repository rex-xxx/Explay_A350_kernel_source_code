...
//			msleep(20);
		...
		}while (((radio->registers[STATUS_REG1] & SEARCH) == 0) && (!timed_out)&&(!search));
		search = 0;
		if (radio->registers[STATUS_REG1] & SEARCH)
...
void power_setup(void)
{
	...
	int rc = 0;
	power = vreg_get(NULL , "msme1");
	if (IS_ERR(power)) {
		pr_err("%s: vreg get failed with : (%ld)\n",
			__func__, PTR_ERR(power));
	}
	rc = vreg_set_level(power, 1800);
	if (rc < 0) {
		pr_err("%s: set regulator level failed with :(%d)\n",
			__func__, rc);
	}
	rc = vreg_enable(power);
	if (rc) {
...
static ssize_t autosearch_show(struct device *dev,struct device_attribute *attr, char *buf)
{	
	int value=0;
	size_t count = 0;
	count += sprintf(buf, "%d\n", value);
...
static ssize_t autosearch_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
	/*
	unsigned long val;
	printk(KERN_ERR"fm power control ret = %d",strict_strtoul(buf, 10, &val));
	if(val == 1)
	{
            power_setup();
	    printk(KERN_ERR"fm power on");
	}else if(val == 0){
            power_shutdown();
	    printk(KERN_ERR"fm power off");
	}
	return count;
	*/
	search = 1;
...
static DEVICE_ATTR(autosearch, 0666, autosearch_show,autosearch_store);

static struct attribute *wcn2243_attributes[] = {
	&dev_attr_autosearch.attr,
	NULL
};


static const struct attribute_group wcn2243_attr_group = {
	.attrs = wcn2243_attributes,
};
...
static int  __init tavarua_probe(struct platform_device *pdev)
{
	...
	private_data = radio;
	ret = sysfs_create_group(&radio->marimba->client->dev.kobj, &wcn2243_attr_group);
	if (ret) {
