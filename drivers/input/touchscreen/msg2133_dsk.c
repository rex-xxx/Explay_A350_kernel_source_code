...
typedef struct
{
    u16 u16X;       /* X coordinate, 0~2047 */
    u16 u16Y;       /* Y coordinate, 0~2047 */
    u8  u8Finger;   /* Number of fingers in the touch event */
    s16 s16Dis_X;   /* X distance between two fingers */
    s16 s16Dis_Y;   /* Y distance between two fingers. This value could be negative */
} touch_data_st;
                                   ...
                                   struct device_attribute *attr, const char *buf, size_t size)
{
    ...
    for (i = 0; i < 1024; i++)
    {
            if(FwDataCnt<59)
		{
		        memcpy(&temp[FwDataCnt*1024], buf, 1024);
		}
...
static void msg21xx_do_work(struct work_struct *work)
{
	...
	if ( ( Checksum == val[7] && val[0] == 0x52 ) )
	{
	        touch_data.u16X = ( val[1] & 0xF0 ) << 4 | val[2];
	        touch_data.u16Y = ( val[1] & 0x0F )<< 8 | val[3];
	        touch_data.s16Dis_X = ( val[4] & 0xF0 ) << 4 | val[5];
	        touch_data.s16Dis_Y = ( val[4] & 0x0F ) << 8 | val[6];
		if((touch_data.s16Dis_X)&0x800)
		{
			touch_data.s16Dis_X|=0xF000;
		}
		if((touch_data.s16Dis_Y)&0x800)
		{
			touch_data.s16Dis_Y|=0xF000;
		}
		copy_x1 = touch_data.u16X*320/2048;
		copy_y1 = touch_data.u16Y*480/2048;
		copy_x2 = (touch_data.u16X+touch_data.s16Dis_X)*320/2048;
		copy_y2 = (touch_data.u16Y+touch_data.s16Dis_Y)*480/2048;
		if ((val[1] == 0xFF) && (val[2] == 0xFF) && (val[3] == 0xFF) )
		{
			if (val[5] != 0x0)
			{
				...
				else if(val[5]&0x08) //TouchKeyMode
				{
					key_value = KEY_MENU;
					#ifdef MSTAR_USE_VIRTUALKEY
					virtualkey_x = 40;
		...
		{
			/* The coordinate report stands for Touch-On-Panel event */
			if ( touch_data.s16Dis_X == 0 && touch_data.s16Dis_Y == 0 )
			{
				if(flag_two_finger)
				{
					...
					TP_DEBUG("one-finger-touch up  x1=%d y1 = %d\n",copy_x2,copy_y2);
					input_sync(input);
					touch_data.u8Finger = 0;
		            	}
				if((copy_x1<=MS_TS_MSG20XX_X_MAX)&&(copy_y1<=MS_TS_MSG20XX_Y_MAX))
				{
					//input_report_abs(input, ABS_MT_TRACKING_ID, 0);
					input_report_abs(input, ABS_MT_TOUCH_MAJOR, 1);
					input_report_abs(input, ABS_MT_POSITION_X, copy_x1);
	            ...
	            else if ((copy_x1<=MS_TS_MSG20XX_X_MAX)&&(copy_y1<=MS_TS_MSG20XX_Y_MAX)&&(copy_x2<=MS_TS_MSG20XX_X_MAX)&&(copy_y2<=MS_TS_MSG20XX_Y_MAX))
	            {
		            	flag_two_finger = 1;
				//input_report_abs(input, ABS_MT_TRACKING_ID, 0);
				input_report_abs(input, ABS_MT_TOUCH_MAJOR, 1);
