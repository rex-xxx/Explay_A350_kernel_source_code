...
void OV5640_set_EV(int ev)
{
     ...
     case 0://+5
         target = AE_Target * 16/5;
         OV5640_set_AE_target(target);
         break;

     case 1://+4
         target = AE_Target * 5/2;
         OV5640_set_AE_target(target);
         break;
     ...
     case 3://+2
         target = AE_Target * 8/5;
         OV5640_set_AE_target(target);
         break;

     case 4://+1
         target = AE_Target * 5/4;
         OV5640_set_AE_target(target);
         break;
     ...
     case 6://-1
         target = AE_Target * 4/5;
         OV5640_set_AE_target(target);
         break;

     case 7://-2
         target = AE_Target * 5/8;
         OV5640_set_AE_target(target);
         break;
     ...
     case 9://-4
         target = AE_Target * 2/5;
         OV5640_set_AE_target(target);
         break;

     case 10://-5
         target = AE_Target * 5/16;
         OV5640_set_AE_target(target);
         break;
...
static int ov5640_video_config(int mode)
{
  ...
  if ((rc = ov5640_sensor_setting(S_UPDATE_PERIODIC,
    ov5640_ctrl->prev_res)) < 0) {
...
static int ov5640_set_contrast(int contrast)
{
    ...
    if (effect_value == CAMERA_EFFECT_OFF)
    {
        ...
        case CAMERA_CONTRAST_LV4:
            ...
            rc = OV5640Core_WritePREG(ov5640_contrast_default_lv4_tbl);
            break;
