...
#include "msm_fb.h"
#include "mipi_dsi.h"
#include "mipi_hx8369a_yassy.h"
#include "mipi_backlight.h"

//static struct msm_panel_common_pdata *mipi_hx8369a_yassy_pdata;
static struct dsi_buf hx8369a_yassy_tx_buf;
static struct dsi_buf hx8369a_yassy_rx_buf;
...
static struct dsi_cmd_desc hx8369a_yassy_cmd_display_on_cmds[] = {
...
static int mipi_hx8369a_yassy_lcd_reset(void)
{
    int rc = 0;
...
static int mipi_hx8369a_yassy_lcd_on(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd;
	struct mipi_panel_info *mipi;

	static int first_in = 0;
	pr_debug("mipi_hx8369a_yassy_lcd_on Start\n");
	...
	pr_debug("%s: mode = %d\n", __func__, mipi->mode);
	if (mipi_hx8369a_yassy_lcd_reset() < 0) {
        pr_debug("mipi_hx8369a_yassy_lcd_reset error\n");
        return -EINVAL;
    }
	msleep(120);
	mipi_dsi_cmds_tx(mfd, &hx8369a_yassy_tx_buf, hx8369a_yassy_cmd_display_on_cmds,
			ARRAY_SIZE(hx8369a_yassy_cmd_display_on_cmds));

	pr_debug("mipi_hx8369a_yassy_lcd_on End\n");

	return 0;
}

static int mipi_hx8369a_yassy_lcd_off(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd;

	pr_debug("mipi_hx8369a_yassy_lcd_off Start\n");
	...
	mipi_dsi_cmds_tx(mfd, &hx8369a_yassy_tx_buf, truly_display_off_cmds,
	...
	pr_debug("mipi_hx8369a_yassy_lcd_off End\n");
...
static int __devinit mipi_hx8369a_yassy_lcd_probe(struct platform_device *pdev)
{
	int ret;
	pr_debug("%s\n", __func__);
...
static struct platform_driver this_driver = {
	.probe  = mipi_hx8369a_yassy_lcd_probe,
	.driver = {
		.name   = "mipi_hx8369a_yassy",
	},
};

static void mipi_hx8369a_yassy_set_backlight(struct msm_fb_data_type *mfd)
{
	mipi_backlight_control(mfd);	
}


static struct msm_fb_panel_data hx8369a_yassy_panel_data = {
	.on	= mipi_hx8369a_yassy_lcd_on,
	.off = mipi_hx8369a_yassy_lcd_off,
	.set_backlight = mipi_hx8369a_yassy_set_backlight,
};

static int ch_used[3];

int mipi_hx8369a_yassy_device_register(struct msm_panel_info *pinfo,
					u32 channel, u32 panel)
{
	struct platform_device *pdev = NULL;
	int ret;

	pr_debug("mipi_hx8369a_yassy_device_register\n");
	...
	pdev = platform_device_alloc("mipi_hx8369a_yassy", (panel << 8)|channel);
	if (!pdev)
		return -ENOMEM;

	hx8369a_yassy_panel_data.panel_info = *pinfo;

	ret = platform_device_add_data(pdev, &hx8369a_yassy_panel_data,
		sizeof(hx8369a_yassy_panel_data));
	if (ret) {
...
static int __init mipi_hx8369a_yassy_lcd_init(void)
{
	pr_debug("mipi_hx8369a_yassy_lcd_init\n");
	
	mipi_dsi_buf_alloc(&hx8369a_yassy_tx_buf, DSI_BUF_SIZE);
	mipi_dsi_buf_alloc(&hx8369a_yassy_rx_buf, DSI_BUF_SIZE);

	return platform_driver_register(&this_driver);
}

module_init(mipi_hx8369a_yassy_lcd_init);
