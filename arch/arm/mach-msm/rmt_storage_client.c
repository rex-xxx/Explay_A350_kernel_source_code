		...
		struct msm_rpc_xdr *xdr)
{
	...
	if (rs_client) {
		pr_info("%s: Handle %d found for %s\n",
	...
	memset((void*)&stats->rd_stats, 0, sizeof(struct rmt_storage_op_stats));
	memset((void*)&stats->wr_stats, 0, sizeof(struct rmt_storage_op_stats));
		...
		struct msm_rpc_xdr *xdr)
{
	...
	pr_info("%s: handle=0x%x size=0x%x\n", __func__, handle, size);
	...
	pr_info("%s: %d bytes at phys=0x%x for handle=%d found\n",
			    ...
			    unsigned long arg)
{
	...
	case RMT_STORAGE_SEND_STATUS:
		...
		op_stats->count++;
		if (curr_stat.tv64 < op_stats->min.tv64)
			op_stats->min = curr_stat;
		if (curr_stat.tv64 > op_stats->max.tv64)
			op_stats->max = curr_stat;
#endif
		pr_info("%s: \thandle=%d err_code=%d data=0x%x\n", __func__,
		...
		size_t count, loff_t *ppos)
{
	...
	for (j = 0; j < tot_clients; j++) {
		...
		if (stats->rd_stats.count)
			i += scnprintf(buf + i, max - i,
				"Avg read time: %lld us\n",
				div_s64(ktime_to_us(stats->rd_stats.total),
				stats->rd_stats.count));
		...
		if (stats->wr_stats.count)
			i += scnprintf(buf + i, max - i,
				"Avg write time: %lld us\n",
				div_s64(ktime_to_us(stats->wr_stats.total),
				stats->wr_stats.count));
	}
...
static int rmt_storage_init_ramfs(struct rmt_storage_srv *srv)
{
	...
	if (ramfs_table->magic_id == RAMFS_INFO_MAGICNUMBER) {
		pr_info("RAMFS table already filled... skipping %s", \
			__func__);
		return 0;
	}
...
static void handle_restart_teardown(struct msm_rpc_client *client)
{
	...
	if (!srv)
		return;
	pr_info("%s: Modem restart for 0x%08x\n", __func__, srv->prog);
...
static void handle_restart_setup(struct msm_rpc_client *client)
{
	...
	if (!srv)
		return;
	pr_info("%s: Scheduling restart for 0x%08x\n", __func__, srv->prog);
...
static int rmt_storage_reg_callbacks(struct msm_rpc_client *client)
{
	...
	pr_info("%s: Callbacks (re)registered for 0x%08x\n\n", __func__,
		 client->prog);
	return 0;
}
...
static int __init rmt_storage_init(void)
{
	...
	pr_info("%s: Shadow memory at %p (phys=%lx), %d bytes\n", __func__,
