...
static void ibs_device_want_to_sleep(struct hci_uart *hu)
{
	...
	default:
		/* any other state is illegal */
		BT_ERR("received HCI_IBS_SLEEP_IND in rx state %ld tx state %ld",
			ibs->rx_ibs_state, ibs->tx_ibs_state);
		break;
	}
...
static inline int ibs_check_data_len(struct ibs_struct *ibs, int len)
{
	...
	} else if (len < 0 || len > room) {
		BT_ERR("Data length is too large or is negative.");
		kfree_skb(ibs->rx_skb);
	} else {
...
static int ibs_recv(struct hci_uart *hu, void *data, int count)
{
	...
	if(count < 0) {
		BT_ERR("hu %p count %d rx_state %ld rx_count %ld",
                        hu, count, ibs->rx_state, ibs->rx_count);
		return count;
	}
