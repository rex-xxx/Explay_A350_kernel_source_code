...
static void mdp_dma2_update_lcd(struct msm_fb_data_type *mfd)
{
	...
	if ((mfd->panel_info.type == MDDI_PANEL) ||
		...
		if(mfd->resume_flag){//resume then flash gray screen
			lenth = iBuf->dma_x * outBpp + iBuf->dma_y * ystride;
			memset((uint8 *) iBuf->buf,  0x00, lenth);
		}
#endif
	} else {
		if (mfd->panel_info.pdest == DISPLAY_1) {
...
void	mdp3_dsi_cmd_dma_busy_wait(struct msm_fb_data_type *mfd)
{
	...
	if (mfd->dma->busy == TRUE) {
		dma_is_busy = inp32(MIPI_DSI_BASE + 0004);
		if(dma_is_busy & 0x02){//DMA BUSY
			if (busy_wait_cnt == 0)
	...
	if (need_wait) {
		...
		if(wait_for_completion_timeout(&mfd->dma->comp, 100000L) == 0){
			pr_info("Macle %s, %s dma busy and timeout\n", __FILE__, __FUNCTION__);
		}
	}
}
#endif
