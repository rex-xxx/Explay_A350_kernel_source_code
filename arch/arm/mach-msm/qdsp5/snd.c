...
#define SND_SET_VOICE_PARA_PROC 42
...
struct snd_set_voc_para_msg{
	struct rpc_request_hdr hdr;
	struct voice_config voc_cfg[3];
};
...
static long snd_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	struct snd_set_voc_para_msg voc_msg;
	...
	case SND_GET_ENDPOINT:
		...
		break;
	case SND_SET_VOICE_PARA:
		if (copy_from_user(&(voc_msg.voc_cfg[0]), (void __user *) arg, sizeof(struct voice_config)*3)) {
			MM_ERR("set voice para: invalid pointer\n");
			rc = -EFAULT;
			break;
		}
		MM_ERR("songzhou:set voice para rpc\n");
		rc = msm_rpc_call(snd->ept,
		SND_SET_VOICE_PARA_PROC,
		&voc_msg, sizeof(voc_msg), 5 * HZ);
