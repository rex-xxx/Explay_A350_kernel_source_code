...
{
	printk(KERN_DEBUG "%s: Matching dev->kobj.name = %s for driver->name = %s\n", __func__, dev->kobj.name, driver->name);
	return !strncmp(dev->kobj.name, driver->name, strlen(dev->kobj.name));
}
