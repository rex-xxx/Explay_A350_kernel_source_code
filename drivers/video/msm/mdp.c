...
int mdp_start_histogram(struct fb_info *info)
{
	...
	if (mdp_suspended) {
		printk(KERN_ERR "%s: mdp suspended, do not start histogram\n", __func__);
		ret = -EPERM;
...
static int mdp_do_histogram(struct fb_info *info, struct mdp_histogram *hist)
{
	...
	/* return if freeze signal or histogram interrupt recieved */
...
EXPORT_SYMBOL(mdp_driver_late_init);
