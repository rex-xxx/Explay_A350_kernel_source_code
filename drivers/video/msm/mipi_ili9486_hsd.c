...
#include "msm_fb.h"
#include "mipi_dsi.h"
#include "mipi_ili9486_hsd.h"
#include "mipi_backlight.h"

static int current_mode = 0;
static struct dsi_buf ili9486_hsd_tx_buf;
static struct dsi_buf ili9486_hsd_rx_buf;
...
static char mipi_ili9486_F2[]={0XF2, 0x18, 0xa3, 0x12, 0x02, 0xb2, 0x12, 0xff, 0x10, 0x00};
...
static char mipi_ili9486_E0[]={0xE0, 0x00, 0x0D, 0x11, 0x04, 0x13, 0x0A, 0x37, 0x98, 0x42, 0x08, 0x09, 0x0C, 0x18, 0x25, 0x0F};
static char mipi_ili9486_E1[]={0xE1, 0x00, 0x1a, 0x17, 0x03, 0x0a, 0x05, 0x2f, 0x75, 0x47, 0x05, 0x0D, 0x0B, 0x2e, 0x32, 0x0F};
static char mipi_ili9486_B1[]={0XB1, 0xB0, 0x11};
static char mipi_ili9486_C5[]={0XC5, 0x00, 0x00,0x80};
static char mipi_ili9486_C1[]={0XC1, 0x41};
...
static struct dsi_cmd_desc ili9486_hsd_cmd_display_on_cmds[] = {
	...
	{DTYPE_DCS_WRITE, 1, 0, 0, SLEEP_OFF_DELAY, sizeof(mipi_ili9486_11), mipi_ili9486_11},
	{DTYPE_DCS_WRITE, 1, 0, 0, DISPLAY_ON_DELAY, sizeof(mipi_ili9486_29), mipi_ili9486_29},
};
static struct dsi_cmd_desc ili9486_hsd_video_display_on_cmds[] = {
	{DTYPE_GEN_LWRITE, 1, 0, 0, CMD_DELAY, sizeof(mipi_ili9486_F7), mipi_ili9486_F7},
	{DTYPE_GEN_LWRITE, 1, 0, 0, CMD_DELAY, sizeof(mipi_ili9486_F1), mipi_ili9486_F1},	
	{DTYPE_GEN_LWRITE, 1, 0, 0, CMD_DELAY, sizeof(mipi_ili9486_F2), mipi_ili9486_F2},	
	{DTYPE_GEN_LWRITE, 1, 0, 0, CMD_DELAY, sizeof(mipi_ili9486_F8), mipi_ili9486_F8},
...
static int mipi_ili9486_hsd_lcd_reset(void)
{
    int rc = 0;
...
static int mipi_ili9486_hsd_lcd_on(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd;
	struct mipi_panel_info *mipi;
	static int first_in = 0;
	pr_debug("mipi_ili9486_hsd_lcd_on Start\n");
	...
	pr_debug("%s: mode = %d\n", __func__, mipi->mode);
	if (mipi_ili9486_hsd_lcd_reset() < 0) {
        pr_debug("mipi_ili9486_hsd_lcd_reset error\n");
        return -EINVAL;
    }

	if(current_mode == 0){		
		pr_info("%s, command mode\n\r", __func__);
		mipi_dsi_cmds_tx(mfd, &ili9486_hsd_tx_buf, ili9486_hsd_cmd_display_on_cmds,
				ARRAY_SIZE(ili9486_hsd_cmd_display_on_cmds));
	}else{
		pr_info("%s, video mode\n\r", __func__);
		mipi_dsi_cmds_tx(mfd, &ili9486_hsd_tx_buf, ili9486_hsd_video_display_on_cmds,
					ARRAY_SIZE(ili9486_hsd_video_display_on_cmds));
	}

	pr_debug("mipi_ili9486_hsd_lcd_on End\n");

	return 0;
}

static int mipi_ili9486_hsd_lcd_off(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd;

	pr_debug("mipi_ili9486_hsd_lcd_off Start\n");
	...
	mipi_dsi_cmds_tx(mfd, &ili9486_hsd_tx_buf, ili9486_display_off_cmds,
	...
	pr_debug("mipi_ili9486_hsd_lcd_off End\n");
...
static int __devinit mipi_ili9486_hsd_lcd_probe(struct platform_device *pdev)
{
	int ret;
	pr_debug("%s\n", __func__);
...
static struct platform_driver this_driver = {
	.probe  = mipi_ili9486_hsd_lcd_probe,
	.driver = {
		.name   = "mipi_ili9486_hsd",
	},
};

static void mipi_ili9486_hsd_set_backlight(struct msm_fb_data_type *mfd)
{
	mipi_backlight_control(mfd);	
}

static struct msm_fb_panel_data ili9486_hsd_panel_data = {
	.on	= mipi_ili9486_hsd_lcd_on,
	.off = mipi_ili9486_hsd_lcd_off,
	.set_backlight = mipi_ili9486_hsd_set_backlight,
};
					...
					u32 channel, u32 panel)
{
	...
	pr_debug("mipi_ili9486_hsd_device_register\n");
	...
	pdev = platform_device_alloc("mipi_ili9486_hsd", (panel << 8)|channel);
	if (!pdev)
		return -ENOMEM;

	ili9486_hsd_panel_data.panel_info = *pinfo;

	ret = platform_device_add_data(pdev, &ili9486_hsd_panel_data,
		sizeof(ili9486_hsd_panel_data));
	if (ret) {
...
static int __init mipi_ili9486_hsd_lcd_init(void)
{
	pr_debug("mipi_ili9486_hsd_lcd_init\n");
	mipi_dsi_buf_alloc(&ili9486_hsd_tx_buf, DSI_BUF_SIZE);
	mipi_dsi_buf_alloc(&ili9486_hsd_rx_buf, DSI_BUF_SIZE);
	return platform_driver_register(&this_driver);
}

module_init(mipi_ili9486_hsd_lcd_init);
