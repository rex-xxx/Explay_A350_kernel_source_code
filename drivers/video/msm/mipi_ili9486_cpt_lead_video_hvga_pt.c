...
static int __init mipi_video_ili9486_cpt_lead_hvga_pt_init(void)
{
	int ret;

#ifdef CONFIG_FB_MSM_MIPI_PANEL_DETECT
	if (msm_fb_detect_client("mipi_video_ili9486_cpt_lead_hvga"))
		return 0;
#endif
	...
	ret = mipi_ili9486_cpt_lead_device_register(&pinfo, MIPI_DSI_PRIM,
						MIPI_DSI_PANEL_VGA);
	
	mipi_ili9486_cpt_lead_sel_mode(1); //select video mode
	if (ret)
...
module_init(mipi_video_ili9486_cpt_lead_hvga_pt_init);
