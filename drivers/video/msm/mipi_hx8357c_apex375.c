/* Copyright (c) 2008-2011, Longcheer. All rights reserved.
*
****Driver IC	: HX8357C
****Panel	: apex
****Size		: 3.75''
...
#include "msm_fb.h"
#include "mipi_dsi.h"
#include "mipi_hx8357c_apex375.h"
#include "mipi_backlight.h"

static int current_mode = 0;
static struct dsi_buf hx8357c_apex375_tx_buf;
static struct dsi_buf hx8357c_apex375_rx_buf;

#define GPIO_AW551_LCD_RESET  129
#define CMD_DELAY 0
#define MIPI_SETTING_DELAY 5
#define SLEEP_OFF_DELAY 250
...
static char hx8357c_gamma_setting[] = {
	0xE0, 0x00,	0x00, 0x02,// 1
	0x00, 0x08, 0x00, 0x11,// 2, 3
	0x00, 0x23, 0x00, 0x2c,// 4, 5
	0x00, 0x40, 0x00, 0x4a,//6, 7
	0x00, 0x52, 0x00, 0x48,//8, 9
	0x00, 0x41, 0x00, 0x3c,// 10, 11
	0x00, 0x33, 0x00, 0x2E,// 12, 13
	0x00, 0x28, 0x00, 0x27,//14 , 15
	0x00, 0x1b, 0x01, 0x02, //16, 17
	0x00, 0x08, 0x00, 0x11, //18, 19
	0x00, 0x23, 0x00, 0x2c, 	// 20, 21
	0x00, 0x40, 0x00, 0x4a, 	//22, 23
	0x00, 0x52, 0x00, 0x48, 	//24, 25
	0x00, 0x41, 0x00, 0x3c, //26, 27
	0x00, 0x33, 0x00, 0x2e, //28, 29
	0x00, 0x28, 0x00, 0x27, //30, 31
	0x00, 0x1b, 0x00, 0x00, //32, 33
	0x00, 0x01,  // 34
};
...
static struct dsi_cmd_desc hx8357c_apex375_cmd_display_on_cmds[] = {
	...
	{DTYPE_GEN_LWRITE, 1, 0, 0, CMD_DELAY, sizeof(hx8357c_cmd_pixel_format_setting), hx8357c_cmd_pixel_format_setting},
...
static struct dsi_cmd_desc hx8357c_apex375_video_display_on_cmds[] = {
	{DTYPE_GEN_LWRITE, 1, 0, 0, CMD_DELAY, sizeof(hx8357c_extention_cmd_enable), hx8357c_extention_cmd_enable},		
	{DTYPE_GEN_LWRITE, 1, 0, 0, CMD_DELAY, sizeof(hx8357c_ba), hx8357c_ba},
	...
	{DTYPE_GEN_LWRITE, 1, 0, 0, CMD_DELAY, sizeof(hx8357c_gamma_setting), hx8357c_gamma_setting},
	{DTYPE_GEN_LWRITE, 1, 0, 0, DISPLAY_ON_DELAY, sizeof(hx8357c_cmd_pixel_format_setting), hx8357c_cmd_pixel_format_setting},	
	{DTYPE_GEN_LWRITE, 1, 0, 0, DISPLAY_ON_DELAY, sizeof(hx8357c_cmd_e9), hx8357c_cmd_e9},
	{DTYPE_DCS_WRITE, 1, 0, 0, SLEEP_OFF_DELAY, sizeof(hx8357c_sleep_out), hx8357c_sleep_out},
	{DTYPE_DCS_WRITE, 1, 0, 0, DISPLAY_ON_DELAY, sizeof(hx8357c_display_on), hx8357c_display_on},
};;

static int mipi_hx8357c_apex375_lcd_reset(void)
{
    int rc = 0;
...
static int mipi_hx8357c_apex375_lcd_on(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd;
	struct mipi_panel_info *mipi;
	static int first_in = 0;
	pr_debug("mipi_hx8357c_apex375_lcd_on Start\n");
	...
	pr_debug("%s: mode = %d\n", __func__, mipi->mode);
	if (mipi_hx8357c_apex375_lcd_reset() < 0) {
        pr_debug("mipi_hx8357c_apex375_lcd_reset error\n");
        return -EINVAL;
    }

	if(current_mode == 0){		
		pr_info("%s, command mode", __func__);
		mipi_dsi_cmds_tx(mfd, &hx8357c_apex375_tx_buf, hx8357c_apex375_cmd_display_on_cmds,
				ARRAY_SIZE(hx8357c_apex375_cmd_display_on_cmds));
	}else{
		pr_info("%s, video mode", __func__);
		mipi_dsi_cmds_tx(mfd, &hx8357c_apex375_tx_buf, hx8357c_apex375_video_display_on_cmds,
					ARRAY_SIZE(hx8357c_apex375_video_display_on_cmds));
	}

	pr_debug("mipi_hx8357c_apex375_lcd_on End\n");

	return 0;
}

static int mipi_hx8357c_apex375_lcd_off(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd;

	pr_debug("mipi_hx8357c_apex375_lcd_off Start\n");
	...
	mipi_dsi_cmds_tx(mfd, &hx8357c_apex375_tx_buf, hx8357c_display_off_cmds,
	...
	pr_debug("mipi_hx8357c_apex375_lcd_off End\n");
...
static int __devinit mipi_hx8357c_apex375_lcd_probe(struct platform_device *pdev)
{
	int ret;
	pr_debug("%s\n", __func__);
...
static struct platform_driver this_driver = {
	.probe  = mipi_hx8357c_apex375_lcd_probe,
	.driver = {
		.name   = "mipi_hx8357c_apex375",
	},
};

static void mipi_hx8357c_apex375_set_backlight(struct msm_fb_data_type *mfd)
{
	mipi_backlight_control(mfd);	
}

static struct msm_fb_panel_data hx8357c_apex375_panel_data = {
	.on	= mipi_hx8357c_apex375_lcd_on,
	.off = mipi_hx8357c_apex375_lcd_off,
	.set_backlight = mipi_hx8357c_apex375_set_backlight,
};
					...
					u32 channel, u32 panel)
{
	...
	pr_debug("mipi_hx8357c_apex375_device_register\n");
	...
	pdev = platform_device_alloc("mipi_hx8357c_apex375", (panel << 8)|channel);
	if (!pdev)
		return -ENOMEM;

	hx8357c_apex375_panel_data.panel_info = *pinfo;

	ret = platform_device_add_data(pdev, &hx8357c_apex375_panel_data,
		sizeof(hx8357c_apex375_panel_data));
	if (ret) {
...
static int __init mipi_hx8357c_apex375_lcd_init(void)
{
	pr_debug("mipi_hx8357c_apex375_lcd_init\n");
	mipi_dsi_buf_alloc(&hx8357c_apex375_tx_buf, DSI_BUF_SIZE);
	mipi_dsi_buf_alloc(&hx8357c_apex375_rx_buf, DSI_BUF_SIZE);
	return platform_driver_register(&this_driver);
}

module_init(mipi_hx8357c_apex375_lcd_init);
