...
#define MMC328X_DEV_NAME	"magdev"
...
static unsigned int mmc328x_poll(struct file *fp, poll_table * wait)
{
	//return 0;
...
#endif
long mmc328x_power_init(void)
{
	struct vreg *vreg_avdd;
	...
	switch(hw_version)
	{
		case 'A':
			{
				vreg_avdd = vreg_get(NULL, "gp6");
			}
			break;
		case 'B':
			{
				vreg_avdd = vreg_get(NULL, "rftx");
			}
			break;
		default:
				vreg_avdd = vreg_get(NULL, "rftx");
	...
	if (IS_ERR(vreg_avdd)) {
		printk(KERN_ERR "%s: vreg get failed (%ld)\n",
		       __func__, PTR_ERR(vreg_avdd));
		return PTR_ERR(vreg_avdd);
	}

	/* units of mV, steps of 50 mV */
#if defined(CONFIG_LCT_AW70)
	rc = vreg_set_level(vreg_avdd, 3000);
#else
	rc = vreg_set_level(vreg_avdd, 2850);
#endif
	if (rc) {
	...
	/*---------------------init vdd------------------------------*/
	vreg_vdd = vreg_get(NULL, "msmp");
	if (IS_ERR(vreg_vdd)) {
		printk(KERN_ERR "%s: vreg get failed (%ld)\n",
		       __func__, PTR_ERR(vreg_vdd));
		return PTR_ERR(vreg_vdd);
	}

	/* units of mV, steps of 50 mV */
	rc = vreg_set_level(vreg_vdd, 2600);
	if (rc) {
		printk(KERN_ERR "%s: vreg set level failed (%d)\n",
			__func__, rc);
		return -EIO;
