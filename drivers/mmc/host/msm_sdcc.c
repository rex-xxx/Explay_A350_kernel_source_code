		...
		if (host->curr.data && host->dma.sg &&
			...
			if (!cmd->error && host->prog_enable) {
				if (status & MCI_PROGDONE) {
					...
					host->prog_enable = 0;
					msmsdcc_request_end(host, cmd->mrq);
					if ((cmd->opcode == 5) && (host->pdev_id == 3)) 
						pr_info("%s: host->pdev_id is %d sleep/awake busy done\n", 
							mmc_hostname(host->mmc), host->pdev_id);
				} else
					host->curr.cmd = cmd;
			} else {
				if (host->prog_enable) {
					...
					host->prog_enable = 0;
					if ((cmd->opcode == 5) && (host->pdev_id == 3)) 
						pr_err("%s: host->pdev_id is %d sleep/awake busy error\n", 
							mmc_hostname(host->mmc), host->pdev_id);
				}
				if (host->dummy_52_needed)
					host->dummy_52_needed = 0;
...
msmsdcc_runtime_suspend(struct device *dev)
{
	...
	if (mmc) {
		...
		pm_runtime_get_noresume(dev);
       	...
       	if (host->mmc && mmc->card && mmc->card->type != MMC_TYPE_SDIO)
		{
