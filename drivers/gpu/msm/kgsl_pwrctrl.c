	...
	if (state == KGSL_PWRFLAGS_OFF) {
		...
		if (!test_and_set_bit(KGSL_PWRFLAGS_AXI_ON,
			...
			KGSL_PWR_INFO(device,
				"axi on, device %d\n", device->id);
			if (pwr->ebi1_clk) {
				//clk_enable(pwr->ebi1_clk);//deleted by Wenke Ma, 2012-4-17
				clk_set_rate(pwr->ebi1_clk,
					pwr->pwrlevels[pwr->active_pwrlevel].
					bus_freq);
				clk_enable(pwr->ebi1_clk);//added by Wenke Ma, 2012-4-17
			}
			if (pwr->pcl)
				msm_bus_scale_client_update_request(pwr->pcl,
					pwr->pwrlevels[pwr->active_pwrlevel].
...
sleep:
	device->ftbl->suspend_context(device);//added by Wenke Ma, 2012-4-17
	device->ftbl->stop(device);//added by Wenke Ma, 2012-4-17
...
{
	int status;//added by Wenke Ma, 2012-4-17
