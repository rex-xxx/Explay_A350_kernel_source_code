...
int mipi_cmd_truly_wvga_pt_late_init(void)
{
	int ret;

#ifdef CONFIG_FB_MSM_MIPI_PANEL_DETECT
	if (msm_fb_detect_client("mipi_cmd_truly_wvga"))
		return 0;
#endif
...
EXPORT_SYMBOL(mipi_cmd_truly_wvga_pt_late_init);

static int __init mipi_cmd_truly_wvga_pt_init(void)
{
  if (boot_splash) {
...
module_init(mipi_cmd_truly_wvga_pt_init);
