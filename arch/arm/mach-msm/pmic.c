...
#define LED_SET_STATE_BACKLIGHT 255
...
//zhaozy porting start
int pmic_led_set_stat(enum mpp_which which,
								 u32 on_delay,
								 u32 off_delay)
{
	return pmic_rpc_set_only(which,on_delay,off_delay,0,3,
					LED_SET_STATE);
}
EXPORT_SYMBOL(pmic_led_set_stat);
//zhaozy porting end
...
int pmic_led_set_stat_backlight(enum mpp_which which,
	                             u32 on_delay,
	                             u32 off_delay)
{
	printk(KERN_ERR"======call remote ========");
	return pmic_rpc_set_only(which,on_delay,off_delay,0,3,
		            LED_SET_STATE_BACKLIGHT);
}
EXPORT_SYMBOL(pmic_led_set_stat_backlight);
