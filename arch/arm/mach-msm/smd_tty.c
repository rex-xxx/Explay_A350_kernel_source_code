...
static int smd_tty_dummy_probe(struct platform_device *pdev)
{
	if (!strncmp(pdev->name, smd_ch_name[0],
		...
		complete_all(&smd_tty[8].ch_allocated);//zhaozy porting
	else if (!strncmp(pdev->name, smd_ch_name[9],
				strnlen(smd_ch_name[9], SMD_MAX_CH_NAME_LEN)))
		complete_all(&smd_tty[9].ch_allocated);//zhaozy porting
...
static int __init smd_tty_init(void)
{
	...
	tty_register_device(smd_tty_driver, 8, 0);//zhaozy porting
	tty_register_device(smd_tty_driver, 9, 0);//zhaozy porting
	...
	init_completion(&smd_tty[8].ch_allocated);//zhaozy porting
	init_completion(&smd_tty[9].ch_allocated);//zhaozy porting
	...
	smd_tty[8].is_open = 0;
	init_waitqueue_head(&smd_tty[8].ch_opened_wait_queue);
	...
	if (ret)
		goto unreg9;//unreg7 //zhaozy porting
...
unreg9:
	platform_driver_unregister(&smd_tty[9].driver);//zhaozy porting
unreg8:
	platform_driver_unregister(&smd_tty[8].driver);//zhaozy porting
unreg7:
...
out:
	...
	tty_unregister_device(smd_tty_driver, 4);
	tty_unregister_device(smd_tty_driver, 7);
	tty_unregister_device(smd_tty_driver, 8);//zhaozy porting
	tty_unregister_device(smd_tty_driver, 9);//zhaozy porting
