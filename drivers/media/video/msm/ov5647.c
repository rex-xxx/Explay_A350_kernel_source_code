...
static void ov5647_get_pict_fps(uint16_t fps, uint16_t *pfps)
{
	...
	printk(KERN_ERR "ov5647_get_pict_fps divider = %d", divider );
...
static int32_t ov5647_set_default_focus(uint8_t af_step)
{
	...
	printk(KERN_ERR "ov5647_set_default_focus:lens pos = %d", ov5647_ctrl->curr_lens_pos);
	rc = ov5647_af_i2c_write_b_sensor(code_val_msb, code_val_lsb);
	if ( rc != 0) // DAC setting
...
static int ov5647_set_flash_light(enum led_brightness brightness)
{
  ...
  // printk("--CAMERA-- ov5647_set_flash_light brightness = %d\n", brightness);

    if (brightness ==LED_FULL)//high
    {
    ...
    pmapp_disp_flashlight_set(brightness);
...
static const char *ov5647Vendor = "OmniVision";
static const char *ov5647NAME = "ov5647";
static const char *ov5647Size = "5M";
		...
		struct device_attribute *attr, char *buf)
{
	...
	sprintf(buf, "%s %s %s\n", ov5647Vendor, ov5647NAME, ov5647Size);
	ret = strlen(buf) + 1;

	return ret;
}
static ssize_t sensor_flash_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	char *after;
	ssize_t rc = 0;
	/* added this for camera flash factory mode test
	 * state : 0 (off)  1(low) 2(high)
	 */
	unsigned long state = simple_strtoul(buf, &after, 10);    
       ov5647_set_flash_light(state);
        #if 0
	msm_camera_flash_set_led_state(
	((struct msm_camera_sensor_info *)ov5647_pdev->dev.platform_data)->flash_data, state);

    #endif
    
	/* above is added at 2010-1-28 */
	return rc;
}

static DEVICE_ATTR(sensor, 0666, sensor_vendor_show, sensor_flash_store);

static struct kobject *android_ov5647;

static int ov5647_sysfs_init(void)
{
	int ret ;

	android_ov5647 = kobject_create_and_add("android_camera", NULL);
	if (android_ov5647 == NULL) {
		pr_info("ov5647_sysfs_init: subsystem_register " \
		"failed\n");
		ret = -ENOMEM;
		return ret ;
	}
	ret = sysfs_create_file(android_ov5647, &dev_attr_sensor.attr);
	if (ret) {
		pr_info("ov5647_sysfs_init: sysfs_create_file " \
		"failed\n");
		kobject_del(android_ov5647);
	}
	return 0 ;
}
		...
		int res)
{
	...
	case SENSOR_SNAPSHOT_MODE:
              ov5647_set_flash_light(LED_FULL);
