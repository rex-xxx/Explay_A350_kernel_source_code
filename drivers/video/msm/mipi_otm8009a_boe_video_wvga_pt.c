...
static int __init mipi_cmd_otm8009a_boe_wvga_pt_init(void)
{
	int ret;

#ifdef CONFIG_FB_MSM_MIPI_PANEL_DETECT
	if (msm_fb_detect_client("mipi_video_otm8009a_boe_wvga"))
		return 0;
#endif
	...
	pinfo.lcdc.v_back_porch = 14;//5;
	pinfo.lcdc.v_front_porch = 16;//5;
	pinfo.lcdc.v_pulse_width = 2;//5;
	...
	ret = mipi_otm8009a_boe_device_register(&pinfo, MIPI_DSI_PRIM,
						MIPI_DSI_PANEL_VGA);
	
	mipi_otm8009a_boe_sel_mode(1); //select video mode
	if (ret)
...
module_init(mipi_cmd_otm8009a_boe_wvga_pt_init);
