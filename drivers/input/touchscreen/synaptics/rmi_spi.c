...
#define RMI_DEFAULT_BYTE_DELAY_US	0 /* 65 /* 65 microseconds inter-byte delay between bytes for RMI chip*/
	...
	if (n_rx) {
		if (instance_data->byte_delay_us) {
			...
			for (;xfer_index < nXfers; xfer_index++) {
				memset(&xfer_list[xfer_index], 0, sizeof(struct spi_transfer));
				xfer_list[xfer_index].len = 1;
				xfer_list[xfer_index].delay_usecs = instance_data->byte_delay_us;
				xfer_list[xfer_index].rx_buf = local_buf + buffer_offset;
				buffer_offset++;
				spi_message_add_tail(&xfer_list[xfer_index], &message);
#ifdef CONFIG_ARCH_OMAP
				printk(KERN_INFO "%s: Did you compensate for ARCH_OMAP?", __func__);
...
// 		x[1].len = n_rx;
			...
			memset(&xfer_list[xfer_index], 0, sizeof(struct spi_transfer));
#ifdef CONFIG_ARCH_OMAP
			xfer_list[xfer_index].len = n_rx-1;	/* since OMAP has one dummy byte. */
#else
	...
	printk(KERN_INFO "%s: Ready to go, xfer_index = %d, nXfers = %d.", __func__, xfer_index, nXfers);
	...
	if (status == 0) {
...
#if COMM_DEBUG
		if (n_rx) {
			printk(KERN_INFO "%s: SPI received %d bytes...", __func__, n_rx);
			for (i=0; i<n_rx; i++) {
		...
		printk(KERN_ERR "%s: spi_sync failed with error code %d.", __func__, status);
	}
...
{
	struct spi_device_instance_data *id = container_of(pd, struct spi_device_instance_data, rpd);
...
static irqreturn_t spi_attn_isr(int irq, void *info)
{
	...
	if (instance_data->rpd.attention)
		instance_data->rpd.attention(&instance_data->rpd, instance_data->instance_no);
	return IRQ_HANDLED;
}

/* TODO: Move this to rmi_bus, and call a function to get the next sensor ID */
...
static int __devinit rmi_spi_probe(struct spi_device *spi)
{
	...
	/* This should have already been set up in the board file, shouldn't it? */
	spi->bits_per_word = 8;
	...
	if (retval < 0) {
		printk(KERN_ERR "%s: spi_setup failed with %d.", __func__, retval);
		return retval;
	}

	buf = kzalloc(SPI_BUFFER_SIZE, GFP_KERNEL);
	if (!buf) {
		printk(KERN_ERR "%s: Failed to allocate memory for spi buffer.", __func__);
		return -ENOMEM;
	}

	instance_data = kzalloc(sizeof(*instance_data), GFP_KERNEL);
	if (!instance_data) {
		printk(KERN_ERR "%s: Failer to allocate memory for instance data.", __func__);
		return -ENOMEM;
	}
	...
	instance_data->rpd.polling_required = true; /* default to polling if irq not used */
	...
	if (platformdata->chip == RMI_SUPPORT) {
		...
		dev_set_name(&spi->dev, "%s%d", RMI4_SPI_DEVICE_NAME, instance_data->instance_no);
		/*
...
#if 0
			retval = request_irq(instance_data->irq, spi_attn_isr, irqtype, "rmi_spi", instance_data);
			if (retval) {
				dev_info(&spi->dev, "%s: Unable to get attn irq %d.  Reverting to polling.", __func__, instance_data->irq);
	...
	retval = rmi_register_sensor(&instance_data->rpd, platformdata->sensordata);
	if (retval) {
		printk(KERN_ERR "%s: sensor registration failed with code %d.", __func__, retval);
		goto error_exit;
	}
	...
	if (instance_data->rpd.polling_required == false) {
		...
		if (retval) {
			dev_err(&spi->dev, "%s: failed to obtain IRQ %d. Result: %d.",
	...
	printk(KERN_INFO "%s: Successfully Registered %s.", __func__, instance_data->rpd.name);
...
rmi_spi_plat_probe(struct platform_device *dev)
{
	...
	if (!platform_data) {
		printk(KERN_ERR "A platform device must contain rmi_spi_platformdata\n");
		return -ENXIO;
	}
...
{
	int retval;
	printk(KERN_INFO "%s: RMI SPI physical layer initialization.", __func__);
#if 1
	retval = spi_register_driver(&rmi_spi_driver);
	if (retval < 0) {
		printk(KERN_ERR "%s: Failed to register spi driver, code = %d.", __func__, retval);
		return retval;
	}
#else
	retval = platform_driver_register(&rmi_spi_platform_driver);
	if (retval < 0) {
		printk(KERN_ERR "%s: Failed to register platform driver, code = %d.", __func__, retval);
		return retval;
	}
#endif
