...
#define TIMEREMOTE_PROCEEDURE_ALARM_SET_PROC 14
...
static int msmrtc_alarm_proc_args(struct msm_rpc_client *client, void *buff,  
								void *data)  
{  
	struct rtc_alarm_args *rtc_args = data;  
	if (rtc_args->proc == TIMEREMOTE_PROCEEDURE_ALARM_SET_PROC) {  
		*(uint32_t *)buff = (uint32_t) cpu_to_be32(rtc_args->alarm_time);  
		pr_info("%s:%d set hw rtc time to %d S after\n", __func__, __LINE__, rtc_args->alarm_time);  
		return sizeof(uint32_t);  
	} else  
		return 0;  
} 
...
int msmrtc_ioctl(struct device *dev, unsigned int cmd, unsigned long arg)  
{  
	int rc = 0;  
	switch (cmd) {  
		case RTC_HW_CMD:  
			rc = msmrtc_timeremote_alarm_set(dev,(uint32_t)arg);  
			break;  
		default:  
			break;  
	}  
