...
struct msm_nand_chip {
	...
	uint8_t *dma_buffer;
	dma_addr_t dma_addr;
	unsigned CFG0, CFG1, CFG0_RAW, CFG1_RAW,CFG0_M,CFG1_M;
...
static int msm_nand_read_modem(struct mtd_info *mtd, loff_t from,
			     struct mtd_oob_ops *ops)
{
	struct msm_nand_chip *chip = mtd->priv;
	...
	if (ops->datbuf != NULL && (ops->len % mtd->writesize) != 0) {
		/* when ops->datbuf is NULL, ops->len can be  */
		pr_err("%s: unsupported ops->len, %d\n",
		       __func__, ops->len);
		return -EINVAL;
	}
	...
	if (ops->datbuf) {
		data_dma_addr_curr = data_dma_addr =
			msm_nand_dma_map(chip->dev, ops->datbuf, ops->len,
				       DMA_FROM_DEVICE);
		if (dma_mapping_error(chip->dev, data_dma_addr)) {
			pr_err("msm_nand_read_modem: failed to get dma addr "
			       "for %p\n", ops->datbuf);
			return -EIO;
		}
	}
	...
	while (page_count-- > 0) {
		...
		dma_buffer->data.cmd = MSM_NAND_CMD_PAGE_READ_ECC;
		dma_buffer->data.cfg0 = (chip->CFG0_M& ~(7U << 6)) | ((cwperpage-1) << 6);
		dma_buffer->data.cfg1 = chip->CFG1_M;		
		...
		for (n = start_sector; n < cwperpage; n++) {
			...
			if (n == start_sector) {
				...
				dma_buffer->data.ecccfg =0x1ff;// mtd->ecc_cfg;
				cmd->cmd = 0;
				cmd->src = msm_virt_to_dma(chip,
						&dma_buffer->data.ecccfg);
				cmd->dst = MSM_NAND_EBI2_ECC_BUF_CFG;
			...
			sectordatasize = 512;//516;
			cmd->cmd = 0;
			cmd->src = MSM_NAND_FLASH_BUFFER;
			cmd->dst = data_dma_addr_curr;
			data_dma_addr_curr += sectordatasize;
			cmd->len = 512;// (n < (cwperpage - 1))? 516 : (512 - ((cwperpage - 1) << 2));
			cmd++;
...
#if VERBOSE
		if (rawerr && !pageerr) {
			pr_err("msm_nand_read_modem %llx %x %x empty page\n",
			       (loff_t)page * mtd->writesize, ops->len,
			       ops->ooblen);
		} else {
			for (n = start_sector; n < cwperpage; n++)
	      ...
	      size_t *retlen, u_char *buf)
{
	...
	ops.datbuf = buf;
	ops.oobbuf = NULL;
	if(mtd->ecc_cfg == 0x1ff)
	{
		ret =  msm_nand_read_modem(mtd, from, &ops);
	}else
	{
...
static int
msm_nand_write_modem(struct mtd_info *mtd, loff_t to,struct mtd_oob_ops *ops)
{
    	struct msm_nand_chip *chip = mtd->priv;
	struct {
		dmov_s cmd[8 * 7 + 2];
	...
	while (page_count-- > 0) {
		...
		dma_buffer->data.cfg0 = chip->CFG0_M;
		dma_buffer->data.cfg1 = chip->CFG1_M;
		...
		for (n = 0; n < cwperpage ; n++) {
			...
			if (n == 0) {
				...
				dma_buffer->data.ecccfg = mtd->ecc_cfg;
	...
	if (ops->datbuf)
		dma_unmap_page(chip->dev, data_dma_addr, ops->len,
				DMA_TO_DEVICE);
	if (err)
		pr_err("msm_nand_write_modem %llx %x failed %d\n",
		       to, ops->len,  err);

#if VERBOSE
		pr_info("\n%s: ret %d, retlen %d \n",
				__func__, err, ops->retlen);
			  ...
			  size_t *retlen, const u_char *buf)
{
	...
	if(mtd->ecc_cfg==0x1FF)
	{
	   	ret =  msm_nand_write_modem(mtd,to,&ops);
	}
	else
	{
		if (!dual_nand_ctlr_present)
...
msm_nand_erase_modem(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	dma_buffer->data.exec = 1;
	dma_buffer->data.flash_status = 0xeeeeeeee;
	dma_buffer->data.cfg0 = chip->CFG0_M & (~(7 << 6));  /* CW_PER_PAGE = 0 */
	dma_buffer->data.cfg1 = chip->CFG1_M;
	dma_buffer->data.clrfstatus = 0x00000020;
	dma_buffer->data.clrrstatus = 0x000000C0;
...
msm_nand_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	unsigned page = 0;
      ...
      if(mtd->ecc_cfg==0x1FF)
	{
	    return msm_nand_erase_modem(mtd,instr);
	}
...
msm_nand_block_isbad(struct mtd_info *mtd, loff_t ofs)
{
    ...
    pr_err("[%s] result 0x%x\n",__func__,dma_buffer->data.result.flash_status);
		ret = -EIO;
    }
...
int msm_nand_scan(struct mtd_info *mtd, int maxchips)
{
	...
	chip->CFG0_M = (((mtd_writesize >> 9) - 1) <<  6)  /* 4/8 cw/pg for 2/4k */
	...
	pr_info("CFG0_M Init  : 0x%08x\n", chip->CFG0_M);
	pr_info("CFG1_M Init  : 0x%08x\n", chip->CFG1_M);
...
//lct start;from and75
//wangzhengguang edit for total memory info
	add_mtd_device(&info->mtd);
//lct end;
}
#else
