...
static ssize_t show_sd_pwer(struct device *dev,
  
                                       struct device_attribute *attr, char *buf)
{
	struct mmc_host *host = cls_dev_to_mmc_host(dev);

       printk("show_sd_pwer:%d\n",(int)host->ios.vdd);
       sprintf(buf, "show_sd_pwer %d\n",(int)host->ios.vdd);
       return 0;
}

//extern  void mmc_power_off(struct mmc_host *host);
extern int msm_sdcc_setup_vreg1(int dev_id, unsigned int enable);
 
static ssize_t set_sd_pwer(struct device *dev,
                                        ...
                                        const char *buf, size_t count)
{
	...
	if(val)   
         msm_sdcc_setup_vreg1(1,1);
	else
         msm_sdcc_setup_vreg1(1,0);
	
       printk("luxiaotong set_sd_pwer:%d\n",(int)val);
	   
	//sprintf(buf, "set_sd_pwer val %d\n", val);
	
       return count;
}


static DEVICE_ATTR(sd_power, S_IRUGO | S_IWUSR,
	show_sd_pwer, set_sd_pwer);
