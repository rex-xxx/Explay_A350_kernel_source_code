...
static int __init mipi_video_hx8357c_hsd_truly375_hvga_pt_init(void)
{
	int ret;

#ifdef CONFIG_FB_MSM_MIPI_PANEL_DETECT
	if (msm_fb_detect_client("mipi_video_hx8357c_hsd_truly375_hvga"))
		return 0;
#endif
	pr_err("%s: to register device!\n", __func__);
	...
	ret = mipi_hx8357c_hsd_truly375_device_register(&pinfo, MIPI_DSI_PRIM,
						MIPI_DSI_PANEL_VGA);
	
	mipi_hx8357c_hsd_truly375_sel_mode(1); //select video mode
	if (ret)
...
module_init(mipi_video_hx8357c_hsd_truly375_hvga_pt_init);
