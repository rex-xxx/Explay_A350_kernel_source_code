...
static u16 diag_enable = 1;
static int android_set_diag_enable(const char *val, struct kernel_param *kp);
static int android_get_diag_enable(char *buffer, struct kernel_param *kp);
module_param_call(diag_enable, android_set_diag_enable, android_get_diag_enable,
					&diag_enable, 0664);
MODULE_PARM_DESC(diag_enable, "usb diag function enable");
...
static int android_set_diag_enable(const char *val, struct kernel_param *kp)
{
	...
	/* We come here even before diag_probe, when diag enable
	...
	if(tmp) {
		...
		if (dev->ch.notify){
			printk(KERN_INFO "android_set_diag_enable: going to notify USB_DIAG_DISCONNECT \n");
...
static int android_get_diag_enable(char *buffer, struct kernel_param *kp)
{
	...
	spin_lock_irqsave(&dev->lock , flags);
	ret = sprintf(buffer, "%x", diag_enable);
	spin_unlock_irqrestore(&dev->lock , flags);
...
void lct_process_diag_enable(u16 enable)
{
	struct diag_context *dev = _context;
	struct usb_diag_ch *ch;
	unsigned long flags;
	
	if (diag_enable == enable)
		goto out;
	...
	if(enable) {
		...
		if (dev->ch.notify) {
			printk(KERN_INFO "lct_process_diag_enable: going to notify USB_DIAG_DISCONNECT \n");
			dev->ch.notify(dev->ch.priv, USB_DIAG_DISCONNECT, NULL);
		}
	}
...
static void usb_config_work_func(struct work_struct *work)
{
	...
	if (ctxt->ch.notify) {
		printk(KERN_INFO "usb_config_work_func: going to notify USB_DIAG_CONNECT \n");
	...
	//printk(KERN_INFO "going to update product id and serial number \n");
	//printk(KERN_INFO "usb composite devices serial number is %d \n", cdev->desc.iSerialNumber);
	...
	for (s = table->strings; s ; s++)
		if (s->id == cdev->desc.iSerialNumber) {
			if(s->s == NULL) {
				//printk(KERN_INFO "serial number is NULL so set serial number to arm null \n");
				/*ctxt->pdata->update_pid_and_serial_num(
					cdev->desc.idProduct, NULL);*/
				msm_hsusb_is_serial_num_null(TRUE);
				/* Send product ID to A9 for software download*/
				//printk(KERN_INFO "usb composite devices product id is 0x%x \n", cdev->desc.idProduct);
				if (cdev->desc.idProduct)
					msm_hsusb_send_productID(cdev->desc.idProduct);
				}
			else {
				//printk(KERN_INFO "usb composite devices serial number is %s \n", s->s);
				ctxt->pdata->update_pid_and_serial_num(
		...
		unsigned intf, unsigned alt)
{
	...
	printk(KERN_INFO "diag_function_set_alt diag_enable value is %d\n", diag_enable);
	if(diag_enable) {
	schedule_work(&dev->config_work);
...
#ifdef CONFIG_GET_USB_INFO_FROM_NV
//struct msm_rpc_client *oem_get_diag_client;
extern struct msm_rpc_client *oem_get_pid_client;

static u16 get_usb_diag_info_from_modem_nv( void )
{
	...
	arg.event = OEM_RAPI_CLIENT_GET_DIAG_INFO;
	arg.cb_func = NULL;
	arg.handle = 0;
	...
	pr_info("*** data obtained from modem %s \n", ret.output);
	...
	if(!strncmp(output, "YS", 2))
		temp = 1;
	else if(!strncmp(output, "NO", 2))
		temp =0;
	else {
		temp =1;
...
#if 0
	oem_get_diag_client = oem_rapi_client_init();
/*** check for error if client failed to initialize. ***/
	if (NULL == oem_get_diag_client) {
	...
	#ifdef CONFIG_GET_USB_INFO_FROM_NV
	diag_enable = get_usb_diag_info_from_modem_nv();
	#endif
