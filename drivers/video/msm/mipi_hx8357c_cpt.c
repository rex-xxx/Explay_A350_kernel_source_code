...
#include "msm_fb.h"
#include "mipi_dsi.h"
#include "mipi_hx8357c_cpt.h"
#include "mipi_backlight.h"

static int current_mode = 0;
static struct dsi_buf hx8357c_cpt_tx_buf;
static struct dsi_buf hx8357c_cpt_rx_buf;

#define GPIO_AW551_LCD_RESET  129
#define CMD_DELAY 0
#define CMD_DELAY_5MS 5
...
static char hx8357c_tear_on[2] = {0x35, 0x00};
...
static char hx8357c_video_vcom[] = {0xB6, 0x44};

static char hx8357c_internal_osc[] = {
...
static char hx8357c_video_disp_cycle[8] = {
	0xB4, 0x01, 0x40, 0x00,
	0x2A, 0x2A, 0x0D, 0x4F
};

static char hx8357c_b5[4] = {
	0xB5, 0x01, 0x01, 0x67
};
...
static struct dsi_cmd_desc hx8357c_cpt_cmd_display_on_cmds[] = {
	...
	{DTYPE_DCS_WRITE, 1, 0, 0, CMD_DELAY, sizeof(hx8357c_tear_on), hx8357c_tear_on},
	...
	{DTYPE_GEN_LWRITE, 1, 0, 0, CMD_DELAY, sizeof(hx8357c_b5), hx8357c_b5},
	{DTYPE_GEN_LWRITE, 1, 0, 0, CMD_DELAY, sizeof(hx8357c_cmd_rgb_interface), hx8357c_cmd_rgb_interface},
	{DTYPE_GEN_LWRITE, 1, 0, 0, CMD_DELAY, sizeof(hx8357c_e3), hx8357c_e3},
	...
	{DTYPE_GEN_LWRITE, 1, 0, 0, CMD_DELAY_5MS, sizeof(hx8357c_cmd_pixel_format_setting), hx8357c_cmd_pixel_format_setting},
	//{DTYPE_GEN_LWRITE, 1, 0, 0, CMD_DELAY_5MS, sizeof(hx8357c_cmd_e9), hx8357c_cmd_e9},	
...
static struct dsi_cmd_desc hx8357c_cpt_video_display_on_cmds[] = {
	{DTYPE_GEN_LWRITE, 1, 0, 0, CMD_DELAY, sizeof(hx8357c_extention_cmd_enable), hx8357c_extention_cmd_enable},		
	{DTYPE_GEN_LWRITE, 1, 0, 0, CMD_DELAY, sizeof(hx8357c_video_vcom), hx8357c_video_vcom},
	{DTYPE_DCS_WRITE, 1, 0, 0, CMD_DELAY, sizeof(hx8357c_tear_on), hx8357c_tear_on},
	{DTYPE_GEN_LWRITE, 1, 0, 0, CMD_DELAY, sizeof(hx8357c_internal_osc), hx8357c_internal_osc},
	{DTYPE_GEN_LWRITE, 1, 0, 0, CMD_DELAY, sizeof(hx8357c_set_panel), hx8357c_set_panel},
	{DTYPE_GEN_LWRITE, 1, 0, 0, MIPI_SETTING_DELAY, sizeof(hx8357c_video_power_ctrl), hx8357c_video_power_ctrl},
	{DTYPE_GEN_LWRITE, 1, 0, 0, CMD_DELAY, sizeof(hx8357c_video_disp_cycle), hx8357c_video_disp_cycle},
	{DTYPE_GEN_LWRITE, 1, 0, 0, CMD_DELAY, sizeof(hx8357c_b5), hx8357c_b5},
...
static int mipi_hx8357c_cpt_lcd_reset(void)
{
    int rc = 0;
...
static int mipi_hx8357c_cpt_lcd_on(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd;
	struct mipi_panel_info *mipi;
	static int first_in = 0;
	pr_debug("mipi_hx8357c_cpt_lcd_on Start\n");
	...
	pr_debug("%s: mode = %d\n", __func__, mipi->mode);
	if (mipi_hx8357c_cpt_lcd_reset() < 0) {
        pr_debug("mipi_hx8357c_cpt_lcd_reset error\n");
        return -EINVAL;
    }
	if(current_mode == 0){		
		pr_info("%s, command mode", __func__);
		mipi_dsi_cmds_tx(mfd, &hx8357c_cpt_tx_buf, hx8357c_cpt_cmd_display_on_cmds,
				ARRAY_SIZE(hx8357c_cpt_cmd_display_on_cmds));
	}else{
		pr_info("%s, video mode", __func__);
		mipi_dsi_cmds_tx(mfd, &hx8357c_cpt_tx_buf, hx8357c_cpt_video_display_on_cmds,
					ARRAY_SIZE(hx8357c_cpt_video_display_on_cmds));
	}

	pr_debug("mipi_hx8357c_cpt_lcd_on End\n");

	return 0;
}

static int mipi_hx8357c_cpt_lcd_off(struct platform_device *pdev)
{
	struct msm_fb_data_type *mfd;

	pr_debug("mipi_hx8357c_cpt_lcd_off Start\n");
	...
	mipi_dsi_cmds_tx(mfd, &hx8357c_cpt_tx_buf, hx8357c_display_off_cmds,
...
#endif
	gpio_set_value_cansleep(GPIO_AW551_LCD_RESET, 0);
	pr_debug("mipi_hx8357c_cpt_lcd_off End\n");
...
static int __devinit mipi_hx8357c_cpt_lcd_probe(struct platform_device *pdev)
{
	int ret;
	pr_debug("%s\n", __func__);
...
static struct platform_driver this_driver = {
	.probe  = mipi_hx8357c_cpt_lcd_probe,
	.driver = {
		.name   = "mipi_hx8357c_cpt",
	},
};

static void mipi_hx8357c_cpt_set_backlight(struct msm_fb_data_type *mfd)
{
	mipi_backlight_control(mfd);	
}

static struct msm_fb_panel_data hx8357c_cpt_panel_data = {
	.on	= mipi_hx8357c_cpt_lcd_on,
	.off = mipi_hx8357c_cpt_lcd_off,
	.set_backlight = mipi_hx8357c_cpt_set_backlight,
};
					...
					u32 channel, u32 panel)
{
	...
	pr_debug("mipi_hx8357c_cpt_device_register\n");
	...
	pdev = platform_device_alloc("mipi_hx8357c_cpt", (panel << 8)|channel);
	if (!pdev)
		return -ENOMEM;

	hx8357c_cpt_panel_data.panel_info = *pinfo;

	ret = platform_device_add_data(pdev, &hx8357c_cpt_panel_data,
		sizeof(hx8357c_cpt_panel_data));
	if (ret) {
...
static int __init mipi_hx8357c_cpt_lcd_init(void)
{
	pr_debug("mipi_hx8357c_cpt_lcd_init\n");
	mipi_dsi_buf_alloc(&hx8357c_cpt_tx_buf, DSI_BUF_SIZE);
	mipi_dsi_buf_alloc(&hx8357c_cpt_rx_buf, DSI_BUF_SIZE);
	return platform_driver_register(&this_driver);
}

module_init(mipi_hx8357c_cpt_lcd_init);
