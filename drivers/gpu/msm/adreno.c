...
#define MAX_WAITGPU_SECS (10 * HZ)  //modify by Wenke Ma at 2012-4-17    (HZ + HZ/2)
...
//	adreno_regwrite(device, REG_RBBM_PM_OVERRIDE1, 0);
	adreno_regwrite(device, REG_RBBM_PM_OVERRIDE1, 0xFFFFFFFE);//modify by Wenke Ma, 2012-4-47
	if (!adreno_is_a220(adreno_dev))
		adreno_regwrite(device, REG_RBBM_PM_OVERRIDE2, 0xFFF);//modify by Wenke Ma, 2012-4-47
//		adreno_regwrite(device, REG_RBBM_PM_OVERRIDE2, 0);
	...
	KGSL_PWR_INFO(device, "device started status %d ts %x\n", status, adreno_dev->ringbuffer.timestamp);//added by Wenke Ma, 2012-4-17
...
{
	struct adreno_device *adreno_dev = ADRENO_DEVICE(device);
	KGSL_PWR_INFO(device, "device stopped state %x\n", device->state);
	if (device->state == KGSL_STATE_SLEEP)//added by Wenke Ma, 2012-4-17
		return 0;
	...
	kgsl_pwrctrl_disable(device);
	KGSL_PWR_INFO(device, "device stopped ts %x\n", adreno_dev->ringbuffer.timestamp);//added by Wenke Ma, 2012-4-17
...
/*
**added by Wenke Ma, for kgsl error 2012-4-16
*/
...
retry:
	if (rb->flags & KGSL_FLAGS_STARTED) {
		do {
			adreno_poke(device);//added by Wenke Ma, for kgsl error 2012-4-16
	...
	if (device->state & (KGSL_STATE_SLEEP | KGSL_STATE_SUSPEND))//added by Wenke Ma. 2012-4-17
				...
				unsigned int msecs)
{
	long status = 0;
	struct adreno_device *adreno_dev = ADRENO_DEVICE(device);
	int retries;//added by Wenke Ma, for kgsl error 2012-4-16
	unsigned int msecs_first;//added by Wenke Ma, for kgsl error 2012-4-16
	unsigned int msecs_part;//added by Wenke Ma, for kgsl error 2012-4-16
...
#else//modify by Wenke Ma, for kgsl error 2012-4-16
/* Keep the first timeout as 100msecs before rewriting the WPTR, Less visible impact if the
**WPTR has not been updated properly.
*/
