			   ...
			   unsigned int iocmd, unsigned long ioarg)
{
...
#endif /* DIAG over USB */
		success = 1;
    }else if(DIAG_IOCTL_SMD_WRITE==iocmd){//lct.panguangyi
        struct io_para* para = (struct io_para*)ioarg;
        void *buf  = NULL;
        if(para->len>0)
            buf = kzalloc(para->len,GFP_KERNEL);
        else
        {
            para->len = 0;
            pr_err("[%s]length to write is not larger than 0!",__func__);
            return -EFAULT;
        }
        if(buf)
        {
            int err;
            err = copy_from_user(buf,para->buf,para->len);
            para->len = smd_device_write(buf,para->len);
            kfree(buf);
            if(err)
                return -EFAULT;
        }else
    ...
    }else if(DIAG_IOCTL_SMD_READ==iocmd){
        int err = -1;
        struct io_para* para = (struct io_para*)ioarg;
        int len_upper = para->len;
        unsigned char buf[255];
        para->len = smd_device_read(buf,para->len);
        if(len_upper<0)
            len_upper = para->len;
        err = copy_to_user(para->buf,(void*)buf,para->len>len_upper?len_upper:para->len);
