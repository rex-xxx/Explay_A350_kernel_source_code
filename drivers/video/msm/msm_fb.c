...
static DEVICE_ATTR(lcd_name, 0666, msm_fb_lcd_name, NULL);

static struct kobject *msm_lcd_name;
static int msm_lcd_name_create_sysfs(struct platform_device *pdev)
{
	int ret ;
...
#ifdef LCM_SUPPORT_READ_VERSION
	msm_lcd_name_create_sysfs(pdev);
#endif
	return 0;
}
	...
	if (mfd->suspend.panel_power_on) {
		ret =
		     msm_fb_blank_sub(FB_BLANK_UNBLANK, mfd->fbi,
				      mfd->op_enable);
		if (ret)
			MSM_FB_INFO("msm_fb_resume: can't turn on display! ret = %d, op_enable = %d\n", ret, mfd->op_enable);
	}
	...
	case FB_BLANK_UNBLANK:
		if (!mfd->panel_power_on) {
			...
			if (ret == 0) {
				...
				if (mfd->panel_info.type == MIPI_CMD_PANEL){//resume then flash gray screen
					mfd->resume_flag = TRUE;
					mfd->dma_fnc(mfd);
					mfd->resume_flag = FALSE;
				}
...
*/
				msleep(50);
				if ( 0 != mfd->bl_level)//fix the bug that display white screen when calling coming.
				{
						msm_fb_set_backlight(mfd,mfd->bl_level);
				}
			}else{
				pr_info("Macle FB_BLANK_UNBLANK ret = %d", ret);
			}
		}
		break;
	...
	default:
		if (mfd->panel_power_on) {
			...
			if (ret){
				...
				pr_info("Macle FB_BLANK_POWERDOWN or default, ret = %d, curr_pwr_state = %d", ret, curr_pwr_state);
			}
