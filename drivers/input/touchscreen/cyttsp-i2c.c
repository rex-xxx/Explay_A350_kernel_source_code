...
#if defined(CONFIG_LCT_AW535_LCT_B) 
...
#if defined(CONFIG_LCT_AW550_QL) 
#define CYPRESS_SUPPORT_UPDATE		0
#elif defined(CONFIG_LCT_AW550_JPA)
#define CYPRESS_SUPPORT_UPDATE		0
#elif defined(CONFIG_LCT_AW550_GKS)
#define CYPRESS_SUPPORT_UPDATE		0
#elif defined(CONFIG_LCT_AW550_ZBY)
#define CYPRESS_SUPPORT_UPDATE		0
...
#define CYPRESS_SUPPORT_UPDATE		1
#elif defined(CONFIG_LCT_AW535_QL)
#define CYPRESS_SUPPORT_UPDATE		1
#else
#define CYPRESS_SUPPORT_UPDATE		0
...
#if defined(CONFIG_LCT_AW550_QL)||defined(CONFIG_LCT_AW550_JPA)||defined(CONFIG_LCT_AW550_GKS)||defined(CONFIG_LCT_AW550_KLL)
#define CYPRESS_FIX_VER_0X11   // gpg some TP  appid_lo = 0x11  It should be 0x1D
#endif
...
#define CYTTSP_AW550_FEIXUN "cyttspfw_aw550_feixun.fw"
#define CYTTSP_AW550_XJY	   "cyttspfw_aw550_xjy.fw"
#define CYTTSP_AW550_VOTO    "cyttspfw_aw550_voto.fw"
#define CYTTSP_AW550_KLL		"cyttspfw_aw550_kll.fw"
#define CYTTSP_AW550_HYG		"cyttspfw_aw550_hyg.fw"
#define CYTTSP_AW550_MCX		"cyttspfw_aw550_mcx_ofilm.fw"
#define CYTTSP_AW550_DBK_TRULY	"cyttspfw_aw550_dbk_truly.fw"
#define CYTTSP_AW550_DBK_HUANDA	"cyttspfw_aw550_dbk_huanda.fw"
#define CYTTSP_AW535_MAK	"cyttspfw_aw535_mak.fw"
#define CYTTSP_AW535B_YEJI	"cyttspfw_aw535b_yeji.fw"


#define CYTTSP_DEFAULT 		 "cyttspfw.fw"

#define CYTTSP_AW551B_TP_VENDOR_ID 0x26
...
#endif
static	int cyttsp_power_up(void);
...
static ssize_t tp_version_read(struct file *file, char __user *buf, size_t count, loff_t *pos)
{
	...
	printk("%s Enter,cyttsp_vendor_id=%d,cyttsp_firmware_version=%d\n",__func__,cyttsp_vendor_id,cyttsp_firmware_version);
	if(cyttsp_firmware_version < 0)
	{
	...
	else
	{
		...
		#if CYPRESS_SUPPORT_UPDATE
		snprintf(tp_version, 30,"M%xV%x  %s\n", cyttsp_vendor_id,cyttsp_firmware_version,"open");
		#else
...
static DEVICE_ATTR(irq_enable, 0644, cyttsp_irq_status, cyttsp_irq_enable);
...
static DEVICE_ATTR(cyttsp_fw_ver, 0644, cyttsp_fw_show, NULL);

static void cyttsp_exit_bl_mode(struct cyttsp *ts);

static ssize_t cyttsp_power_reset(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t size)
{
	struct i2c_client *client = container_of(dev, struct i2c_client, dev);
	...
	err = strict_strtoul(buf, 10, &value);
	printk(KERN_INFO "%s: err = %d,value=%ld", __func__,err,value);
	if (err != 0)
		return err;
...
static ssize_t cyttsp_tp_mode(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = container_of(dev, struct i2c_client, dev);
	struct cyttsp *ts = i2c_get_clientdata(client);
	...
	if(rc  < 0)
	{
		printk(KERN_ERR"cyttsp_tp_mode read mode error\n");
	}
	else
	{
	   
	   printk(KERN_ERR"cyttsp_tp_mode read mode tmpData[0]:%x   tmpData[1]:%x\n",tmpData[0],tmpData[1]);
	}
	return sprintf(buf, "%x:%x\n", tmpData[0],tmpData[1]);
}


static DEVICE_ATTR(power_reset, 0644, cyttsp_tp_mode, cyttsp_power_reset);
				...
				int data_len, u8 *buf, bool force)
{
	...
	if(rc < 0)
	{
		...
		printk(KERN_ERR"cyttspfw_flash_start tries ;%d\n",tries);
		...
		{
		   printk(KERN_ERR"cyttspfw_flash_start tp tmpData[0] is:%x   tmpData[1]is %x:\n",tmpData[0],tmpData[1]);
		}
...
#ifdef CYPRESS_FIX_VER_0X11		// gpg Cheer TP V04 appid_lo = 0x11  It should be 0x1D
		else if(g_bl_data.appid_lo == 0x11){
			pr_info("Fix Cheer 0x11 appid");
			if (appver_hi > g_bl_data.appver_hi) {
				fw_upgrade = 1;
			} else if ((appver_hi == g_bl_data.appver_hi) &&
					 (appver_lo > g_bl_data.appver_lo)) {
...
static void cyttspfw_upgrade(struct device *dev, bool force)
{
...
#if 1  //lct lusongbai
	   /*cyttsp_vendor_id table
	    			                module          version
	    	AW551-ofilm:              01			    01
	    	AW551-truly:              02				01
	    	AW551-yejiguangdian:      03			    01
	    	AE500-ofilm:              04				01
	    	AE500-shenyue:            05				01
	    	AE550-HE-ofilm:           06				01
	    	AE550-HE-truly:           07				01
	    	AE550-ql-truly:           08				01
	    	AW550-ql-truly:	  0x11				04
	    	AW550-feixun			0x16
	    	AW550-XJY			0x14
	    	AW550-VOTO			0x17
	    */
	    //according to vendor id ,select firmware file
	  printk("%s lusongbai cyttsp_vendor_id=%d \n",__func__,cyttsp_vendor_id);
	...
	#ifdef CONFIG_LCT_AW550_QL
	else if (cyttsp_vendor_id !=0x11){
		force = 1;
		fwfile=CYTTSP_AW550_TRULY;
		printk("dingliang--qile------------\n");
	}
	#endif	

#ifdef CONFIG_LCT_AW550_JPA
	else if (cyttsp_vendor_id !=0x11){
		force = 1;
		fwfile=CYTTSP_AW550_TRULY;
		printk("dingliang--jpa------------\n");
	}
#endif	

#ifdef CONFIG_LCT_AW550_GKS
	else if (cyttsp_vendor_id !=0x11){
		force = 1;
		fwfile=CYTTSP_AW550_TRULY;
		printk("dingliang--GKS------------\n");
	}
#endif
	
	#ifdef CONFIG_LCT_AW550_FXT
	else if (cyttsp_vendor_id !=0x16){
		force = 1;
		fwfile=CYTTSP_AW550_FEIXUN;
		printk("dingliang--feixun------------\n");
	}
	#endif

	#ifdef CONFIG_LCT_AW550_XJY
	else if (cyttsp_vendor_id !=0x14){
		force = 1;
		fwfile=CYTTSP_AW550_XJY;
		printk("dingliang--xjy------------\n");
	}
	#endif

	#ifdef CONFIG_LCT_AW550_ZBY
	else if (cyttsp_vendor_id !=0x17){
		force = 1;
		fwfile=CYTTSP_AW550_VOTO;
		printk("dingliang--voto------------\n");
	}
	#endif
	#ifdef CONFIG_LCT_AW550_KLL
	else if (cyttsp_vendor_id !=0x19){
		force = 1;
		fwfile=CYTTSP_AW550_KLL;
		printk("%s: --kll update------------\n", __func__);
	}
	#endif
	#ifdef CONFIG_LCT_AW550_HYG
	else if(cyttsp_vendor_id !=0x15){
		force = 1;
		fwfile=CYTTSP_AW550_HYG;
		printk("%s: --HYG update------------\n", __func__);
	}
	#endif	
	#ifdef CONFIG_LCT_AW535_MAK
	else if(cyttsp_vendor_id !=0x1B){
		force = 1;
		fwfile=CYTTSP_AW535_MAK;
		printk("%s: --AW535 MAK update------------\n", __func__);
	}
	#endif
	#ifdef CONFIG_LCT_AW535_LCT_B
	else if(cyttsp_vendor_id !=CYTTSP_AW551B_TP_VENDOR_ID){
		force = 1;
		fwfile=CYTTSP_AW535B_YEJI;
		printk("%s: --AW535B LCT update------------\n", __func__);
	}
	#endif
	#ifdef CONFIG_LCT_AW535_QL
	else if(cyttsp_vendor_id !=CYTTSP_AW551B_TP_VENDOR_ID){
		force = 1;
		fwfile=CYTTSP_AW535B_YEJI;
		printk("%s: --AW535B QL update------------\n", __func__);
	}
	#endif
#endif	
	#if 1
	 else if(request_firmware(&cyttsp_fw, "cyttspfw.fw", dev) >= 0){ //if no kernel config
			printk("cyttspfw_upgrade no kernel config project request fw success \n");
			blfwrequested = true;
			fwfile = CYTTSP_DEFAULT;
			if(cyttsp_vendor_id != 0x1B) //aw535
			{
				force = 1;
			}
	   }
	 #endif
	 else if(cyttsp_vendor_id==0x01){  //AW551-ofilm
			   fwfile=CYTTSP_AW551_OFILM;
	  }
	  else if(cyttsp_vendor_id==0x02){//AW551-truly
			   fwfile=CYTTSP_AW551_TRULY;
	  }
	  else if(cyttsp_vendor_id==0x11){//AW550-truly   //QL & JPA & gks
	           fwfile=CYTTSP_AW550_TRULY;
	  }
	  else if(cyttsp_vendor_id==0x16){//AW550-feixun
	           fwfile=CYTTSP_AW550_FEIXUN;
	  }

	  else if(cyttsp_vendor_id==0x14){//AW550-xjy
	           fwfile=CYTTSP_AW550_XJY;
	  }

	  else if(cyttsp_vendor_id==0x17){//AW550-voto
	           fwfile=CYTTSP_AW550_VOTO;
	  }
	  else if(cyttsp_vendor_id==0x19){//AW550-kll
	           fwfile=CYTTSP_AW550_KLL;
	  }
	  else if(cyttsp_vendor_id==0x15){//AW550-HYG
			fwfile=CYTTSP_AW550_HYG;
	  }
	  else if(cyttsp_vendor_id==0x06){//AW550-mcx
			fwfile=CYTTSP_AW550_MCX;
	  }
	  else if(cyttsp_vendor_id==0x1D){//AW550-dbk
			fwfile=CYTTSP_AW550_DBK_TRULY;
	  }
	  else if(cyttsp_vendor_id==0x20){//AW550-dbk
			fwfile=CYTTSP_AW550_DBK_HUANDA;
	  }
	  else if(cyttsp_vendor_id==0x1B){//AW535_MAK
			fwfile=CYTTSP_AW535_MAK;
	  } 
	  else if(cyttsp_vendor_id==CYTTSP_AW551B_TP_VENDOR_ID){//AW535B
			fwfile=CYTTSP_AW535B_YEJI;
	  } 
	  else{
	      printk("%s this vendor 's upgrade NOT SUPPORT !!!!!!!! \n",__func__);
             return;
	  }
	
	 strncpy(ts->fw_fname, fwfile,FW_FNAME_LEN - 1);
		printk("%s lusongbai fw_fname=%s \n",__func__, ts->fw_fname);
  #endif
...
#if !CYPRESS_SUPPORT_UPDATE 	// add by gpg close TP update except for GKS
		return size;
#endif

#ifdef CONFIG_LCT_AW550_MCX
printk("cyttsp_update_fw_store cyttsp_vendor_id=%x,cyttsp_firmware_version=%x\n",cyttsp_vendor_id,cyttsp_firmware_version);
if((cyttsp_vendor_id != 6) )
{
	return size;
}
	...
	if (rc != 0)
		return rc;
       printk("cyttsp_update_fw_store val=%ld\n",val);
	mutex_lock(&ts->mutex);
	if (!ts->cyttsp_fwloader_mode  && val) {
		ts->cyttsp_fwloader_mode = 1;
...
static DEVICE_ATTR(cyttsp_fw_name, 0644, cyttsp_fw_name_show,
		...
		if (ts->platform_data->use_trk_id) {
			...
			for (id = 0; id < CY_NUM_MT_TCH_ID; id++) {
				...
				if (snd_trk[id] < CY_NUM_TRK_ID) {
					...
					if((cur_mt_pos[snd_trk[id]][CY_XPOS] > 330) || (cur_mt_pos[snd_trk[id]][CY_YPOS] > 550))
					{
						printk(KERN_ERR"cyttsp_xy_worker TP report error x:%x,y:%x ,we should reset tp\n",cur_mt_pos[snd_trk[id]][CY_XPOS],cur_mt_pos[snd_trk[id]][CY_YPOS]);
						resetTp = 1;
					}
...
exit_xy_worker:
	if (cyttsp_disable_touch) {
		...
		if (ts->client->irq == 0) {
			...
			if(resetTp == 1)
			{
				printk(KERN_ERR"cyttsp_xy_worker reset tp\n");
				gpio_set_value(ts->platform_data->resout_gpio, 1); //reset high valid
				msleep(50);
				gpio_set_value(ts->platform_data->resout_gpio, 0);
				msleep(100);
...
static irqreturn_t cyttsp_irq(int irq, void *handle)
{
	...
	{
		cyttsp_xdebug("%s: queue work\n",CY_I2C_NAME);
		queue_work(cyttsp_ts_wq, &ts->work);
	}
	else
	{
		printk(KERN_INFO "%s: work pending", __func__);
	}
	return IRQ_HANDLED;
}
...
static int cyttsp_initialize(struct i2c_client *client, struct cyttsp *ts)
{
	...
	set_bit(BTN_MISC,input_device->keybit);
    set_bit(KEY_OK, input_device->keybit);	
	#if 0
	...
	//input_set_capability(input_device, EV_KEY, KEY_OK);
	...
	gpio_set_value(ts->platform_data->resout_gpio,1);
	...
	printk(KERN_INFO "%s: ts->platform_data->irq_gpio:%d", __func__,ts->platform_data->irq_gpio);
	...
	if (1) {
		...
		retval = gpio_request(TP_ID_GPIO,"ts_tp_id");
		if (retval) {
			pr_err("%s: unable to request gpio [%d]\n", __func__,
						TP_ID_GPIO);
			goto error_tpid_gpio_req;
		}
		retval = gpio_direction_input(ts->platform_data->irq_gpio);
		if (retval) {
			pr_err("%s: unable to set_direction for gpio [%d]\n",
					__func__, TP_ID_GPIO);
			goto error_tpid_gpio_dir;
		}
	}
	...
	#endif
	{
	    ...
	    */
		...
		if(nRetryCount >= 5)
		{
			printk(KERN_ERR"cyttsp_initialize read version and module error\n");
		}
		else
		{
		       cyttsp_vendor_id=tmpData[16];
			   cyttsp_firmware_version = tmpData[17];
			printk(KERN_ERR"cyttsp_initialize tp module is:%x   firmware version is %x:\n",tmpData[16],tmpData[17]);
		}
	}
	...
	#if 1
	ts->tpid = gpio_get_value(TP_ID_GPIO);
	printk(KERN_ERR"cyttsp_initialize ts->tpid:%d\n",ts->tpid);
	if(ts->tpid == 1)
	{
		#ifdef CONFIG_LCT_AW551_YL
		//cyttsp_vendor_id = 2;
		#endif
		printk(KERN_ERR"cyttsp_initialize it is truly moudle\n");
	}
	else
	{
		#ifdef CONFIG_LCT_AW551_YL
		//cyttsp_vendor_id = 1;
		#endif
		printk(KERN_ERR"cyttsp_initialize it is O-film moudle\n");
	}
	#endif
	...
	if (retval) {
		cyttsp_alert("sysfs entry for power reset failed\n");
		goto error_rm_dev_file_power_reset;
	}
	#endif
...
#ifdef CYTTSP_SUPPORT_SYS_NODE
error_rm_dev_file_power_reset:
	device_remove_file(&client->dev, &dev_attr_power_reset);
...
error_power_device:
	...
	#if 0
error_tpid_gpio_dir:
	if (gpio_is_valid(TP_ID_GPIO))
		gpio_free(TP_ID_GPIO);
error_tpid_gpio_req:
error_irq_gpio_dir:
	if (gpio_is_valid(ts->platform_data->irq_gpio))
...
static	int cyttsp_power_down(void)
{
	...
	printk(KERN_ERR "%s: Power down  VCC success\n", __func__);
	msleep(100);
	printk(KERN_ERR "%s: start power down iovdd\n", __func__);
#if 1
	...
	printk(KERN_ERR "%s: Power down iovdd success\n", __func__);
	msleep(100);
#endif
			...
			const struct i2c_device_id *id)
{
	...
	{
		printk(KERN_ERR"cyttsp_probe other driver has been loaded\n");
		return ENODEV;
	}
	...
	if (!(retval < CY_OK)) {
		...
		#if 1
		do{
			retval = i2c_smbus_read_i2c_block_data(ts->client,CY_REG_BASE,17, tmpData); 
			msleep(200);
			if(retval < 0)
			{
				gpio_set_value(ts->platform_data->resout_gpio, 1); //reset high valid
		  ...
		  if(retval < 0)
		  {
			...
			printk(KERN_ERR"cyttsp_probe the driver is not match the driver\n");
			return -ENODEV;
		
		  }
		  else
		  {		 	
		  	printk(KERN_ERR"cyttsp_probe tmpData[1] = %x\n",tmpData[1]);
			cyttsp_has_bootloader  = ((tmpData[1] & 0x10) >> 4);
			printk(KERN_ERR"cyttsp_probe cyttsp_has_bootloader = %x\n",cyttsp_has_bootloader);		
		  }
		#endif
		retval = CY_OK;
...
static int cyttsp_suspend(struct device *dev)
{
	...
	if (!(retval < CY_OK)) {
		if (ts->platform_data->use_sleep &&
			...
			printk(KERN_ERR"cyttsp_suspend I2C retval:%d\n",retval);
		}
	}
