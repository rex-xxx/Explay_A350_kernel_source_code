...
static int ltr502_light_disable(struct ltr502_data *ltr502)
{
	...
	if (ltr502->power_state & PROXIMITY_ENABLED) {
		usleep(200000);
		hrtimer_start(&ltr502->proximity_timer, ltr502->proximity_poll_delay, HRTIMER_MODE_REL);
	}
	return ret;
}
				...
				const char *buf, size_t size)
{
	...
	if (new_delay != ktime_to_ns(ltr502->light_poll_delay)) {
		...
		if (ltr502->power_state & LIGHT_ENABLED) {
			...
			hrtimer_start(&ltr502->light_timer, ltr502->light_poll_delay, HRTIMER_MODE_REL);
		}
	}
				      ...
				      const char *buf, size_t size)
{
	...
	mutex_lock(&ltr502->lock);
	ltr502_dbgmsg("new_value = %d, old state = %d\n",
		    new_value, (ltr502->power_state & PROXIMITY_ENABLED) ? 1 : 0);
...
static void ltr502_work_func_light(struct work_struct *work)
{
	...
	{
		input_report_abs(ltr502->light_input_dev, ABS_MISC, lux_table[data]*2);
...
{
	struct ltr502_data *ltr502 = container_of(timer, struct ltr502_data, light_timer);
...
static enum hrtimer_restart ltr502_pxy_timer_func(struct hrtimer *timer)
{
	...
	hrtimer_forward_now(&ltr502->proximity_timer, ltr502->proximity_poll_delay);
	return HRTIMER_RESTART;
}
...
static int ltr502_hardware_init(struct ltr502_data *ltr502)
{
	...
	if(rc) 
	{
		pr_err("fail to write REGS_TIMING,try to write REGS_DPS_CTL..\n");
		data = 0x44;
	...
	/*FIXME:DLS Window 93.5% loss for 7x27A version 1.5 due to the high window loss*/ 
	data = 0x0F;
			  ...
			  const struct i2c_device_id *id)
{
	...
	hrtimer_init(&ltr502->proximity_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
...
static void ltr502_early_suspend(struct early_suspend *h)
{
	...
	*/
	u8 mode;
	struct ltr502_data *ltr502 = container_of(h, struct ltr502_data, early_suspend);
